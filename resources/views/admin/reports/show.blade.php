<x-app-layout>
    <div class="py-12 bg-sky-50/60 dark:bg-slate-900 min-h-screen transition-colors duration-300">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            @if(Auth::user()->role === 'admin')
                <div class="bg-slate-800 dark:bg-slate-950 rounded-2xl shadow-lg p-4 flex flex-col sm:flex-row justify-between items-center gap-4 border border-slate-700">
            ...
            <div class="bg-white/90 dark:bg-slate-800/90 backdrop-blur-md shadow-sm sm:rounded-2xl border border-sky-100 dark:border-slate-700 p-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-6 transition-colors duration-300">
                ...
                <div class="text-left md:text-right">
                    ...
                    <p class="font-black text-xl text-sky-700 dark:text-sky-300 mt-0.5 mb-1.5">{{ $report->user->department }} | Shift {{ $report->shift_id }}</p>
                    <p class="text-xs font-bold text-slate-500 dark:text-slate-400">
                        <span class="inline-block bg-slate-50 dark:bg-slate-900/50 px-2.5 py-1 rounded-md border border-slate-200 dark:border-slate-600 shadow-sm">
                            📅 {{ \Carbon\Carbon::parse($report->created_at)->translatedFormat('d M Y') }} &nbsp;|&nbsp; ⏰ {{ \Carbon\Carbon::parse($report->created_at)->format('H:i') }} WIB
                        </span>
                    </p>
                </div>
            </div>
            ...
            @if(session('error'))
                <div class="bg-rose-50 dark:bg-rose-900/30 border border-rose-200 dark:border-rose-800 text-rose-700 dark:text-rose-400 px-5 py-4 rounded-2xl relative shadow-sm font-bold" role="alert">
                    {{ session('error') }}
                </div>
            @endif
            ...
            <div class="bg-white/90 dark:bg-slate-800/90 backdrop-blur-md shadow-md sm:rounded-2xl p-8 border border-sky-100 dark:border-slate-700 transition-colors duration-300">
                <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 border-b border-sky-100 dark:border-slate-700 pb-4 gap-4">
                    <div class="flex items-center gap-4">
                        <a href="{{ url()->previous() !== url()->current() ? url()->previous() : (Auth::user()->role === 'admin' ? route('admin.dashboard') : route('dashboard')) }}" class="inline-flex items-center bg-slate-100 dark:bg-slate-700 hover:bg-slate-200 dark:hover:bg-slate-600 text-slate-700 dark:text-slate-200 font-bold py-2.5 px-4 rounded-xl transition border border-slate-200 dark:border-slate-600 text-sm">
                            &#8592; Kembali
                        </a>
                    ...
                    <div class="flex items-center gap-4">
                        ...
                        @if(Auth::user()->role === 'staff' && Auth::id() == $report->user_id && $report->status === 'planned')
                            <form action="{{ route('reports.destroy', $report->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus laporan ini? Seluruh data yang belum disubmit akan hilang secara permanen.');">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="bg-rose-50 dark:bg-rose-900/30 hover:bg-rose-100 dark:hover:bg-rose-900/50 text-rose-600 dark:text-rose-400 font-bold py-2 px-4 rounded-xl transition text-sm border border-rose-200 dark:border-rose-800">
                                    Hapus Laporan
                                </button>
                            </form>
                        @endif
                    ...
                    <form id="final-report-form" action="{{ route('reports.updateFinal', $report->id) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div id="master-task-container" class="space-y-6">
                            @foreach($report->items as $index => $item)
                                <div class="border border-sky-200 dark:border-sky-800 p-6 rounded-2xl bg-sky-50/40 dark:bg-slate-900/50 relative shadow-sm hover:shadow transition">
                                    <div class="mb-4">
                                        <span class="font-black text-lg text-slate-800 dark:text-white flex items-center gap-2">
                                            <span class="text-sky-600 dark:text-sky-400">{{ $index + 1 }}.</span> {{ $item->is_additional ? ($item->task_name ?? 'Tugas Tambahan') : ($item->task->name ?? 'Tugas') }}
                                        </span>
                                    </div>
                                    ...
                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                                        <div>
                                            ...
                                            <select name="items[{{ $item->id }}][status]" class="w-full rounded-lg border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 text-slate-800 dark:text-white text-sm font-semibold status-select" data-id="{{ $item->id }}">
                                                <option value="completed">✅ Selesai</option>
                                                <option value="pending">⏳ Kendala / Pending</option>
                                            </select>
                                        </div>
                                        <div class="obstacle-div hidden" id="obstacle-{{ $item->id }}">
                                            <label class="block text-xs font-black text-rose-500 dark:text-rose-400 uppercase mb-1">Alasan Kendala</label>
                                            <input type="text" name="items[{{ $item->id }}][obstacle_note]" class="w-full rounded-lg border-rose-300 dark:border-rose-700 bg-rose-50 dark:bg-rose-900/20 text-rose-800 dark:text-rose-200 text-sm placeholder-rose-300 dark:placeholder-rose-500" placeholder="Jelaskan alasan pending...">
                                        </div>
                                    </div>
                                    ...
                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                        <div>
                                            ...
                                            <label class="relative flex flex-col items-center justify-center w-full h-64 border-2 border-dashed border-sky-200 dark:border-sky-700/50 rounded-2xl cursor-pointer bg-white dark:bg-slate-800 hover:bg-sky-50/50 dark:hover:bg-slate-700/50 overflow-hidden group transition">
                                                <div class="upload-placeholder flex-col items-center justify-center pt-5 pb-6 {{ $item->before_image ? 'hidden' : 'flex' }}">
                                                    <span class="text-4xl text-sky-400 dark:text-sky-500 font-bold group-hover:text-sky-600 dark:group-hover:text-sky-400">+</span>
                                                    <p class="text-xs text-slate-500 dark:text-slate-400 font-bold mt-2 group-hover:text-sky-600 dark:group-hover:text-sky-400">Tap Upload Foto</p>
                                                </div>
                                    ...
                                    <div class="mt-6">
                                        ...
                                        <input type="text" name="items[{{ $item->id }}][notes]" value="{{ old('items.'.$item->id.'.notes', $item->notes) }}" class="block w-full border-sky-200 dark:border-slate-600 bg-white dark:bg-slate-800 text-slate-800 dark:text-white font-medium focus:border-sky-500 focus:ring-sky-500 rounded-xl shadow-sm py-3 px-4">
                                    </div>
                                </div>
                            @endforeach
                        </div>
                        ...
                        <div class="mt-8 flex flex-col sm:flex-row justify-between items-center bg-sky-50/80 dark:bg-slate-800/80 p-6 rounded-2xl border border-sky-200 dark:border-slate-700 gap-4 transition-colors duration-300">
                            <a href="{{ route('dashboard') }}" class="w-full sm:w-auto text-center bg-slate-200 dark:bg-slate-700 hover:bg-slate-300 dark:hover:bg-slate-600 text-slate-700 dark:text-slate-200 font-bold py-3.5 px-6 rounded-xl transition shadow-sm">
                                ...
                                <div class="flex gap-2 mt-auto pt-3 border-t border-slate-100 dark:border-slate-700">
                                    <div class="flex-1">
                                        ...
                                        @if($item->before_image)
                                            <img src="{{ asset('storage/' . $item->before_image) }}" onclick="openLightbox(this.src)" class="w-full h-24 object-cover rounded-lg cursor-pointer hover:opacity-80 transition shadow-sm border border-slate-100 dark:border-slate-700">
                                        @else
                                    ...
                                    <div class="flex-1">
                                        ...
                                        @if($item->after_image)
                                            <img src="{{ asset('storage/' . $item->after_image) }}" onclick="openLightbox(this.src)" class="w-full h-24 object-cover rounded-lg cursor-pointer hover:opacity-80 transition shadow-sm border border-slate-100 dark:border-slate-700">
                                        @else
                                            <div class="w-full h-24 bg-slate-100 dark:bg-slate-700/50 rounded-lg flex items-center justify-center text-xs text-slate-400 dark:text-slate-500 font-bold border border-slate-200 dark:border-slate-700">Kosong</div>
                                        @endif
                                    </div>
    ...
    @if(Auth::user()->role === 'staff' && Auth::id() == $report->user_id && $report->status === 'planned')
        <div id="custom-confirm-modal" class="fixed inset-0 z-50 items-center justify-center bg-slate-900/40 backdrop-blur-sm hidden">
            <div class="bg-white dark:bg-slate-800 rounded-3xl p-8 max-w-md w-full mx-4 shadow-2xl border border-sky-100 dark:border-slate-700 text-center transform transition-all">
                <div class="w-16 h-16 bg-sky-100 dark:bg-sky-900/30 text-sky-600 dark:text-sky-400 rounded-2xl flex items-center justify-center mx-auto mb-4 text-3xl shadow-inner border border-sky-200 dark:border-sky-800">
                    🚀
                </div>
                ...
                <div class="flex space-x-3">
                    <button type="button" id="modal-btn-cancel" class="flex-1 bg-slate-100 dark:bg-slate-700 hover:bg-slate-200 dark:hover:bg-slate-600 text-slate-700 dark:text-slate-200 font-bold py-3.5 px-4 rounded-xl transition">
                        Batal
                    </button>
                    ...
                    btnAddAdditional.addEventListener('click', function() {
                        const html = `
                            <div class="border border-amber-300 dark:border-amber-700/50 p-6 rounded-2xl bg-amber-50/40 dark:bg-amber-900/10 relative shadow-sm hover:shadow transition mt-6 additional-item-block">
                                <button type="button" class="absolute top-4 right-4 bg-rose-50 dark:bg-rose-900/30 border border-rose-200 dark:border-rose-800 text-rose-600 dark:text-rose-400 hover:bg-rose-100 dark:hover:bg-rose-900/50 font-bold py-1.5 px-3 rounded-lg text-xs z-10 btn-remove-additional transition">Hapus</button>
                                ...
                                <div class="mb-4 pr-24">
                                    ...
                                    <input type="text" name="new_items[${newTaskCounter}][task_name]" class="block w-full border-amber-200 dark:border-slate-600 bg-white dark:bg-slate-800 text-slate-800 dark:text-white font-medium focus:border-amber-500 focus:ring-amber-500 rounded-xl shadow-sm py-3 px-4" required>
                                </div>

                                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <div>
                                        <p class="block text-xs font-black text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-2">Foto Before</p>
                                        <label class="relative flex flex-col items-center justify-center w-full h-64 border-2 border-dashed border-amber-200 dark:border-amber-700/50 rounded-2xl cursor-pointer bg-white dark:bg-slate-800 hover:bg-amber-50/50 dark:hover:bg-slate-700/50 overflow-hidden group transition">
                                            <div class="upload-placeholder flex flex-col items-center justify-center pt-5 pb-6">
                                                <span class="text-4xl text-amber-400 dark:text-amber-500 font-bold group-hover:text-amber-600 dark:group-hover:text-amber-400">+</span>
                                                <p class="text-xs text-slate-500 dark:text-slate-400 font-bold mt-2 group-hover:text-amber-600 dark:group-hover:text-amber-400">Tap Upload Foto</p>
                                            </div>
                                ...
                                <div class="mt-6">
                                    ...
                                    <input type="text" name="new_items[${newTaskCounter}][notes]" class="block w-full border-amber-200 dark:border-slate-600 bg-white dark:bg-slate-800 text-slate-800 dark:text-white font-medium focus:border-amber-500 focus:ring-amber-500 rounded-xl shadow-sm py-3 px-4">
                                </div>
                            </div>
                        `;
