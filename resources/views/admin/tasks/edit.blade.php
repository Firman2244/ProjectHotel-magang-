<x-app-layout>
    <div class="p-4 md:p-8 space-y-6">
        ...
        <div class="bg-white dark:bg-slate-800 p-6 md:p-8 rounded-3xl shadow-sm border border-slate-200 dark:border-slate-700 max-w-3xl">
            <h2 class="text-2xl font-black text-slate-800 dark:text-white mb-6">Edit SOP Tugas</h2>
            ...
            <form action="{{ route('admin.tasks.update', $task->id) }}" method="POST" class="space-y-6">
                ...
                <div>
                    ...
                    <input type="text" name="name" value="{{ old('name', $task->name) }}" class="w-full bg-slate-50 dark:bg-slate-900 border-slate-200 dark:border-slate-700 text-slate-800 dark:text-white rounded-xl focus:ring-sky-500 focus:border-sky-500 font-semibold text-sm px-4 py-3" required>
                ...
                <div class="flex gap-3 pt-4 border-t border-slate-100 dark:border-slate-700">
                    <button type="submit" class="bg-sky-600 hover:bg-sky-700 text-white font-bold py-3 px-6 rounded-xl shadow-lg shadow-sky-600/30 text-sm">Perbarui Tugas</button>
