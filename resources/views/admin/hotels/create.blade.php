        ...
        <div class="flex-1 flex flex-col min-w-0 overflow-y-auto">
            <header class="bg-sky-50/30 backdrop-blur-md border-b border-sky-100 h-16 flex items-center justify-between px-6 shadow-sm">
                <div class="flex items-center">
                    <span class="font-extrabold text-xl text-blue-700 tracking-tight">Tambah Hotel</span>
                </div>
            </header>
            ...
            <div class="p-8 max-w-3xl space-y-6">
                ...
                <div class="bg-white/90 backdrop-blur-md rounded-2xl shadow-sm border border-sky-200/60 p-6">
                    <form action="{{ route('admin.hotels.store') }}" method="POST" class="space-y-5">
                        @csrf
                        <div>
                            <label class="block text-xs font-black text-slate-600 uppercase tracking-wider mb-2">Nama Hotel</label>
                            <input type="text" name="name" value="{{ old('name') }}" required class="w-full border-sky-200 bg-white text-slate-800 font-semibold rounded-xl shadow-sm py-2.5 px-3 text-sm focus:border-sky-500 focus:ring-sky-500" placeholder="Contoh: Hotel Wahyu">
                        ...
                        <div>
                            ...
                            <textarea name="address" rows="3" class="w-full border-sky-200 bg-white text-slate-800 font-semibold rounded-xl shadow-sm py-2.5 px-3 text-sm focus:border-sky-500 focus:ring-sky-500" placeholder="Masukkan alamat lengkap hotel...">{{ old('address') }}</textarea>
                        ...
                        <div>
                            ...
                            <input type="text" name="phone_number" value="{{ old('phone_number') }}" class="w-full border-sky-200 bg-white text-slate-800 font-semibold rounded-xl shadow-sm py-2.5 px-3 text-sm focus:border-sky-500 focus:ring-sky-500" placeholder="Contoh: 08123456789">
