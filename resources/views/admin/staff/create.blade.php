    ...
    <div class="min-h-screen bg-sky-950/5 flex">
        ...
        <div class="flex-1 flex flex-col min-w-0 overflow-y-auto">
            <header class="bg-sky-50/30 backdrop-blur-md border-b border-sky-100 h-16 flex items-center justify-between px-6 shadow-sm">
                <span class="font-extrabold text-xl text-blue-700 tracking-tight">Tambah Staf Baru</span>
            </header>
                ...
                <div class="bg-white/90 backdrop-blur-md rounded-2xl shadow-sm border border-sky-200/60 p-6">
                    <form action="{{ route('admin.staff.store') }}" method="POST" class="space-y-5">
                        ...
                        <div>
                            ...
                            <input type="text" name="name" value="{{ old('name') }}" required class="w-full border-sky-200 bg-white text-slate-800 font-semibold rounded-xl shadow-sm py-2.5 px-3 text-sm focus:border-sky-500 focus:ring-sky-500" placeholder="Contoh: Budi Karyawan">
                        ...
                        <div>
                            ...
                            <input type="email" name="email" value="{{ old('email') }}" required class="w-full border-sky-200 bg-white text-slate-800 font-semibold rounded-xl shadow-sm py-2.5 px-3 text-sm focus:border-sky-500 focus:ring-sky-500" placeholder="budi@hotel.com">
                            @error('email') <span class="text-xs text-rose-600 font-bold mt-1">{{ $message }}</span> @enderror
                        </div>

                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                <label class="block text-xs font-black text-slate-600 uppercase tracking-wider mb-2">Password</label>
                                <input type="password" name="password" required class="w-full border-sky-200 bg-white text-slate-800 font-semibold rounded-xl shadow-sm py-2.5 px-3 text-sm focus:border-sky-500 focus:ring-sky-500" placeholder="••••••••">
                                @error('password') <span class="text-xs text-rose-600 font-bold mt-1">{{ $message }}</span> @enderror
                            </div>
                            <div>
                                <label class="block text-xs font-black text-slate-600 uppercase tracking-wider mb-2">Konfirmasi Password</label>
                                <input type="password" name="password_confirmation" required class="w-full border-sky-200 bg-white text-slate-800 font-semibold rounded-xl shadow-sm py-2.5 px-3 text-sm focus:border-sky-500 focus:ring-sky-500" placeholder="••••••••">
                            </div>
                        </div>
                        ...
                        <div class="pt-4 flex gap-3">
                            <button type="submit" class="bg-sky-600 hover:bg-sky-700 text-white font-bold py-2.5 px-6 rounded-xl shadow-md transition text-sm">Simpan Akun Staf</button>
