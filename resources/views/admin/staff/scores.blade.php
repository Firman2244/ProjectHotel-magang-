    ...
    <div>
        ...
        <div class="p-4 md:p-8 space-y-6">
            <div class="md:hidden flex items-center justify-between bg-white dark:bg-slate-800 p-4 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700">
                <span class="font-extrabold text-sm text-slate-800 dark:text-white tracking-tight">Leaderboard Skor Staf</span>
            </div>
            ...
            <div class="bg-white dark:bg-slate-800 p-5 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700">
                <form method="GET" action="{{ route('admin.staff.scores') }}" class="flex flex-wrap gap-3 items-end">
                    ...
                    <div>
                        ...
                        <select name="month" class="w-40 text-sm font-semibold border-slate-200 dark:border-slate-600 rounded-xl bg-slate-50 dark:bg-slate-900 text-slate-800 dark:text-white focus:border-sky-500 focus:ring-sky-500">
                    ...
                    <div>
                        ...
                        <select name="year" class="w-32 text-sm font-semibold border-slate-200 dark:border-slate-600 rounded-xl bg-slate-50 dark:bg-slate-900 text-slate-800 dark:text-white focus:border-sky-500 focus:ring-sky-500">
                            @foreach($years as $yr)
                    ...
                    <div class="flex gap-2">
                        <button type="submit" class="bg-sky-600 hover:bg-sky-700 text-white text-sm font-bold py-2.5 px-6 rounded-xl shadow-sm">Tampilkan</button>
                    </div>
                </form>
            </div>
                            ...
                            @forelse($staffs as $index => $staff)
                                <tr class="hover:bg-slate-50/50 dark:hover:bg-slate-700/30">
                                    ...
                                    <td class="px-5 py-3 text-center">
                                        <a href="{{ route('admin.staff.points', $staff['id']) }}" class="inline-flex items-center justify-center bg-sky-100 dark:bg-sky-900/30 text-sky-700 dark:text-sky-400 border border-sky-200 dark:border-sky-800 hover:bg-sky-200 dark:hover:bg-sky-800/50 font-bold py-1.5 px-3 rounded-lg text-xs shadow-sm">
                                            Lihat Histori
                                        </a>
                                    </td>
                                </tr>
