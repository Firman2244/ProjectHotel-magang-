...
class ProcessNoteImage implements ShouldQueue
{
    ...
    // Ditambahkan parameter $column ('image' atau 'resolved_image')
    ...
    public function handle(): void
    {
        ...
        try {
            ...
            $prefix = $this->column === 'resolved_image' ? 'resolved_' : 'note_';
            $finalPath = $folderPath . '/' . $prefix . uniqid() . '_' . Str::random(5) . '.jpg';

            \App\Helpers\processImage(Storage::disk('public')->path($this->tempPath), Storage::disk('public')->path($finalPath), 800);

            $note->update([$this->column => $finalPath]);
        } catch (Exception $e) {
            Log::error("Gagal memproses gambar ({$this->column}) Note ID {$this->noteId}: " . $e->getMessage());
        } finally {
