...
class DashboardController extends Controller
{
    public function index(Request $request)
    {
        ...
        $totalStaff = User::where('role', 'staff')->when($hotelId, fn($q) => $q->where('hotel_id', $hotelId))->count();

        $baseTodayQuery = Report::where('report_date', $today)->when($hotelId, fn($q) => $q->where('hotel_id', $hotelId));
        $submittedCount = (clone $baseTodayQuery)->where('status', 'completed')->count();
        $lateCount = (clone $baseTodayQuery)->where('is_late', true)->count();

        $reports = Report::with(['user', 'items.task'])
